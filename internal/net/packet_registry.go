...
func packetIDFromTypeClientbound(t Type, p Protocol) int32 {
	var idToType map[int32]Type

	switch p {
	case Handshake:
		idToType = HandshakeClientboundIds
		break
	case Status:
		idToType = StatusClientboundIds
		break
	case Login:
		idToType = LoginClientboundIds
		break
	case Play:
		idToType = PlayClientboundIds
		break
	}

	for id, pkt := range idToType {
		if pkt.Name() == t.Name() {
			return id
		}
	}
	panic("packet id not defined for type: " + t.Name())
}

func packetTypeFromIDServerbound(id int32, protocol Protocol) Type {
	var idToType map[int32]Type

	switch protocol {
	case Handshake:
		idToType = HandshakeServerboundIds
		break
	case Status:
		idToType = StatusServerboundIds
		break
	case Login:
		idToType = LoginServerboundIds
		break
	case Play:
		idToType = PlayServerboundIds
		break
	}

	result := idToType[id]
	if result == nil {
		panic(fmt.Sprintf("packet type not defined for ID: 0x%02X", id))
	}
	return result
}
