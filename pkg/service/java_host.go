...
func (s *JavaService) populateDummyData() {
	...
	s.RDB.Set(context.Background(), "config:description", "A JungleTree Server", 0)
	s.RDB.Set(context.Background(), "config:favicon", favicon, 0)
	s.RDB.Set(context.Background(), "config:max_players", 20, 0)
}
